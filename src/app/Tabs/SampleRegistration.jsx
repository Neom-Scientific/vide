...
export const SampleRegistration = () => {
  ...
  const uploadTrf = async (file) => {
    ...
    try {
      if (file) {
        // Send to Python model before preview/save
        const formData = new FormData();
        formData.append('file', file);
        const modelRes = await axios.post('/api/python-model', formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        if (modelRes.data?.status !== 200) {
          toast.error('Model processing failed');
        }
        const url = URL.createObjectURL(file);
        setTrfUrl(url);
        form.setValue('trf', file.name);
        form.setValue('trf_file', file.name);
        // Optionally save parsed output from model to hidden field for backend
        // form.setValue('model_result', JSON.stringify(modelRes.data?.result || {}));
      } else {
        setTrfUrl('');
        form.setValue('trf', '');
